...
export default function AdminDashboard() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    const token = localStorage.getItem('access_token');
    try {
      await fetch('http://localhost:8000/trainers', {
        method: 'POST',
  ...
  const handleRemove = async (id: string) => {
    const token = localStorage.getItem('access_token');
    try {
      await fetch(`http://localhost:8000/trainers/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      fetchTrainers();
  ...
  const handleToggleStall = async (t: Trainer) => {
    ...
    try {
      await fetch(`http://localhost:8000/trainers/${t.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
  ...
  const saveEdit = async () => {
    ...
    try {
      await fetch(`http://localhost:8000/trainers/${editingId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
